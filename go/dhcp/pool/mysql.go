...
func NewMysqlPool(capacity uint64, name string, algorithm int, sql *sql.DB, ctx *context.Context) (PoolBackend, error) {
	dp := Mysql{}
	dp.PoolName = name
	dp.SQL = sql
	dp.NewDHCPPool(capacity, algorithm)
	return &dp, nil
}

func (dp *Mysql) NewDHCPPool(capacity uint64, algorithm int) {
	d := &DHCPPool{
		lock:      &sync.RWMutex{},
	...
	for i := uint64(0); i < capacity; i++ {
		// Need to test err
		rows, _ := dp.SQL.Query("INSERT INTO dhcppool (pool_name, idx, released) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE id=id", dp.PoolName, i, time.Now().UnixNano())
		rows.Close()
	}
	dp.DHCPPool = d
}
